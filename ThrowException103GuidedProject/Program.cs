...
while (transactions > 0)
{
    ...
    try
    {
        ...
        //Console.WriteLine($"Transaction successfully completed.");  Backup Calculation - each transaction adds current "itemCost" to the till
...
{
    int sumAvailableTwenties = cashTill[3] + twenties;
    int sumAvailableTens = cashTill[2] + tens;
    int sumAvailableFives = cashTill[1] + fives;
    int sumAvailableOnes = cashTill[0] + ones;
    ...
    while ((changeNeeded > 19) && (sumAvailableTwenties > 0))
    {
        sumAvailableTwenties--;
        changeNeeded -= 20;
        Console.WriteLine("\t A twenty");
    }

    while ((changeNeeded > 9) && (sumAvailableTens > 0))
    {
        sumAvailableTens--;
        changeNeeded -= 10;
    ...
    while ((changeNeeded > 4) && (sumAvailableFives > 0))
    {
        sumAvailableFives--;
        changeNeeded -= 5;
    ...
    while ((changeNeeded > 0) && (sumAvailableOnes > 0))
    {
        sumAvailableOnes--;
        changeNeeded--;
    ...
    cashTill[0] = sumAvailableOnes;
    cashTill[1] = sumAvailableFives;
    cashTill[2] = sumAvailableTens;
    cashTill[3] = sumAvailableTwenties;
